...
process {
    ...
    (& "$factoryDirectory\FactoryResourceGenerator.exe" /v:+ /f:"$searchDirectory" /of:"$searchDirectory\Factory.bin" /sp:"Aderant*.dll`,*.exe") | Tee-Object -FilePath (Join-Path -Path $logDirectory -ChildPath "FactoryResourceGenerator.log")
}
