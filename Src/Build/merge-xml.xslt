...
<!--
   ...
   This script has been modified from the original but is not
   distributed externally thus does not require to be published under the
   license.
-->
...
<xslt:transform version="1.0"
                ...
                exclude-result-prefixes="m msxsl">
...
<xslt:template match="*">
   <xslt:message>
      <xslt:text />Merging input with '<xslt:value-of select="string(msxsl:node-set($with)/*)" />
      <xslt:text>'</xslt:text>
   </xslt:message>
   <!--
   Modification to ignore xslt validation
   ...
   <xslt:call-template name="m:merge">
      ...
      <!-- Modification to allow passing the XML document as a parameter -->
      <xslt:with-param name="nodes2" select="msxsl:node-set($with)/*" />
   </xslt:call-template>
</xslt:template>
