...
namespace Aderant.Build.Analyzer {
    public static class AnalyzerOptionsExtensions {
        ...
        public static string[] GetConfigurationValues(this AnalyzerOptions options, SyntaxTree syntaxTree, string key, string[] defaultValue) {
            var configurationValue = GetConfigurationValue(options, syntaxTree, key, string.Empty);

            if (!string.IsNullOrWhiteSpace(configurationValue)) {
                string[] values = configurationValue.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Select(s => s.Trim()).ToArray();
                return values;
            }
        ...
        public static bool TryGetConfigurationValue(this AnalyzerOptions options, SyntaxTree syntaxTree, string key, out string value) {
            var configuration = options.AnalyzerConfigOptionsProvider.GetOptions(syntaxTree);
