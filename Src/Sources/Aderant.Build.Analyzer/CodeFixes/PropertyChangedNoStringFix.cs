...
namespace Aderant.Build.Analyzer.CodeFixes {
    ...
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(PropertyChangedNoStringFix)), Shared]
    public class PropertyChangedNoStringFix : CodeFixProvider {

        private const string Title = "Use nameof()";

        public sealed override ImmutableArray<string> FixableDiagnosticIds => ImmutableArray.Create(PropertyChangedNoStringRule.DiagnosticId);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context) {
            ...
            // Find the invocation expression for xxPropertyChanged().
        ...
        private async Task<Document> UseNameofAsync(Document document, InvocationExpressionSyntax invocationExpression, CancellationToken cancellationToken) {
            ...
            PropertyChangedNoStringRule.TryGetPropertyName(invocationExpression, semanticModel, out propertyName, out propertyNameLiteral);
